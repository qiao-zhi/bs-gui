...
public class BeanUtils {
	...
	public static <T> Object[][] beans2TwoDimensionArray(List<T> beans, String[] beanProperties) {
		if (beans == null || beans.size() == 0 || beanProperties == null || beanProperties.length == 0) {
			return new Object[0][];
		}

		int size = beans.size();
		Object[][] array = new Object[size][beanProperties.length];
		Object[] tmpResult = null;
		for (int i = 0; i < size; i++) {// 循环遍历所有行
			tmpResult = new Object[0];
			for (String property : beanProperties) {
				Object value = getProperty(beans.get(i), property);
				tmpResult = ArrayUtils.add(tmpResult, value);
			}
	 ...
	 *            排除的属性
	 */
	public static <T> void copyProperties(T dest, T origin, boolean setNull) {
		try {
			// 获取person类的BeanInfo对象
	...
	public static <T> void copyProperties(T dest, T origin) {
		copyProperties(dest, origin, false);
	}
